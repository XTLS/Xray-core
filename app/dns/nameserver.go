...
func (c *Client) QueryIP(ctx context.Context, domain string, option dns.IPOption) ([]net.IP, uint32, error) {
	...
	if c.expectedIPs != nil && !c.actPrior {
		ips = c.expectedIPs.FilterIPs(ips, false)
	...
	if c.unexpectedIPs != nil && !c.actUnprior {
		ips = c.unexpectedIPs.FilterIPs(ips, true)
	...
	if c.expectedIPs != nil && c.actPrior {
		ipsNew := c.expectedIPs.FilterIPs(ips, false)
		if len(ipsNew) > 0 {
			ips = ipsNew
	...
	if c.unexpectedIPs != nil && c.actUnprior {
		ipsNew := c.unexpectedIPs.FilterIPs(ips, true)
		if len(ipsNew) > 0 {
			ips = ipsNew
