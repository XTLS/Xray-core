...
func AddAddressInfoIfDoesNotExist(target, subTarget, type_ string, isServer bool) {
	...
	if err := i.AddressCol().FindOne(ctx, M{"_id": target}).Decode(&addressRecord); err != nil && !errors.Is(err, mongo.ErrNoDocuments) {
		...
		if err == nil {
			...
			addressRecord.Orgs = addressRecord.Orgs.AppendIfNotEmpty(addressRecord.Orgs...)
			addressRecord.SubTargets = addressRecord.SubTargets.AppendIfNotExistAndNotEmpty(subTarget)
			addressRecord.IsMobile = addressRecord.IsMobile.AppendIfNotExist(addressRecord.IsMobile...)
			addressRecord.IsProxy = addressRecord.IsProxy.AppendIfNotExist(addressRecord.IsProxy...)
