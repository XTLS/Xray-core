...
func applyInboundSocketOptions(network string, fd uintptr, config *SocketConfig) error {
	...
	if isTCPSocket(network) {
		switch config.Tfo {
		case SocketConfig_Enable:
			if err := syscall.SetsockoptInt(int(fd), syscall.IPPROTO_TCP, unix.TCP_FASTOPEN, int(config.TcpFastOpenQlen)); err != nil {
				return newError("failed to set TCP_FASTOPEN=", config.TcpFastOpenQlen).Base(err)
			}
		case SocketConfig_Disable:
			if err := syscall.SetsockoptInt(int(fd), syscall.IPPROTO_TCP, unix.TCP_FASTOPEN, 0); err != nil {
