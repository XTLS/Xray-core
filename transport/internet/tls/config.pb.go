...
type Config struct {
	...
	// SNI to forge in the spoofed ClientHello sent via raw sockets before
	// the real TLS handshake (sing-box style TLS spoofing).
	Spoof string `protobuf:"bytes,23,opt,name=spoof,proto3" json:"spoof,omitempty"`
	// How the spoofed ClientHello is corrupted to evade DPI: "wrong-sequence"
	// (default), "wrong-checksum", "wrong-ack", "wrong-md5" or
	// "wrong-timestamp".
	SpoofMethod string `protobuf:"bytes,24,opt,name=spoof_method,json=spoofMethod,proto3" json:"spoof_method,omitempty"`
	// Number of times to inject the fake ClientHello (0 or 1 = single-shot).
	SpoofCount    int32 `protobuf:"varint,25,opt,name=spoof_count,json=spoofCount,proto3" json:"spoof_count,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Config) GetSpoofMethod() string {
	if x != nil {
...
func (x *Config) GetSpoofCount() int32 {
	if x != nil {
		return x.SpoofCount
	}
	return 0
