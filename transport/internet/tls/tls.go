...
// FingerprintSupportsTLS13 reports whether every ClientHello generated by the
// named fingerprint can negotiate TLS 1.3.
func FingerprintSupportsTLS13(name string) bool {
	fingerprint := GetFingerprint(name)
	if fingerprint == nil {
		return false
	}
	if strings.HasPrefix(fingerprint.Client, "Randomized") &&
		(fingerprint.Weights == nil || fingerprint.Weights.TLSVersMax_Set_VersionTLS13 < 1) {
		return false
	}
	...
	for _, extension := range spec.Extensions {
		if supportedVersions, ok := extension.(*utls.SupportedVersionsExtension); ok {
			return slices.Contains(supportedVersions.Versions, uint16(utls.VersionTLS13))
		}
	}
	return false
}
