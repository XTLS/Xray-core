...
func buildTCPSegment(
	...
	if src.Addr().Is4() {
		...
		ip.Encode(uint16(len(frame)), 0, defaultTTL, TCPProtocolNumber, src.Addr(), dst.Addr())
	} else {
		ipHeaderLen = IPv6MinimumSize
		frame = make([]byte, ipHeaderLen+ipPayloadLen)
		ip := IPv6(frame[:ipHeaderLen])
		ip.Encode(uint16(ipPayloadLen), TCPProtocolNumber, defaultTTL, src.Addr(), dst.Addr())
	}
...
func resolveSpoofPacketInfo(method Method, sendNext, receiveNext, timestamp uint32, tcpOptions, payload []byte) (spoofPacketInfo, error) {
	...
	default:
		return packetInfo, fmt.Errorf("tls_spoof: unknown method %v", method)
	}
