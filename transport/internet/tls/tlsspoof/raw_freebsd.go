...
func newRawSpoofer(conn net.Conn, method Method) (rawSpoofer, error) {
	if method == MethodWrongTimestamp {
		return nil, errors.New("tls_spoof: wrong-timestamp is not supported on FreeBSD")
	}
...
func readFreeBSDTCPSequence(conn *net.TCPConn) (uint32, uint32, error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("tls_spoof: SyscallConn: %w", err)
	}
	...
	err = raw.Control(func(fd uintptr) {
		...
		if errno != 0 {
			sockErr = fmt.Errorf("tls_spoof: getsockopt TCP_INFO: %w", errno)
			return
		}
		if bufLen < freebsdTCPInfoMinSize {
			sockErr = fmt.Errorf("tls_spoof: TCP_INFO too short: %d < %d", bufLen, freebsdTCPInfoMinSize)
			return
		}
...
func openFreeBSDRawSocket(src, dst netip.AddrPort) (int, unix.Sockaddr, error) {
	...
	if err != nil {
		return -1, nil, fmt.Errorf("tls_spoof: open AF_INET6 SOCK_RAW: %w", err)
	}
	err = unix.Bind(fd, &unix.SockaddrInet6{Addr: src.Addr().As16()})
	if err != nil {
		unix.Close(fd)
		return -1, nil, fmt.Errorf("tls_spoof: bind AF_INET6 SOCK_RAW: %w", err)
	}
