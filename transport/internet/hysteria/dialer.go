...
func (c *client) dial() error {
	...
	if len(c.config.Ports) > 0 {
		...
		pktConn, err = udphop.NewUDPHopPacketConn(addr, time.Duration(c.config.IntervalMin)*time.Second, time.Duration(c.config.IntervalMax)*time.Second, c.udphopDialer, pktConn, index)
		if err != nil {
