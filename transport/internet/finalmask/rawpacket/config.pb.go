...
type Config struct {
	...
	// Pre-shared key authenticating and encrypting the tunnel. Required
	// in both modes; must match on client and relay.
	Auth string `protobuf:"bytes,23,opt,name=auth,proto3" json:"auth,omitempty"`
	// Suppress kernel-generated TCP RST for the relay's listen port
	// (Linux only; uses iptables OUTPUT REJECT rules). Prevents the kernel
	// from answering the fake TCP handshake with RST.
	SuppressRst bool `protobuf:"varint,24,opt,name=suppress_rst,json=suppressRst,proto3" json:"suppress_rst,omitempty"`
	// Masquerade for non-tunnel traffic (probes, scanners): "http" or
	// "tls" serve a fake web service to TCP probes, "dns" answers UDP
	// queries. Empty or "off" disables.
	Masquerade    string `protobuf:"bytes,26,opt,name=masquerade,proto3" json:"masquerade,omitempty"`
...
func (x *Config) GetSuppressRst() bool {
	if x != nil {
		return x.SuppressRst
	}
	return false
}

func (x *Config) GetMasquerade() string {
	if x != nil {
		return x.Masquerade
	}
	return ""
