...
func TestEncodeRDataTXT(t *testing.T) {
	...
	if len(encoded) < 1 {
		t.Errorf("EncodeRDataTXT(%v) returned %v, want at least 1 byte", p, encoded)
	}
	...
	nilEncoded := EncodeRDataTXT(nil)
	if len(nilEncoded) < 1 {
		t.Errorf("EncodeRDataTXT(nil) returned %v, want at least 1 byte", nilEncoded)
	}
}
...
func TestResolverModeRoundTrip(t *testing.T) {
	...
	// Mock resolver: bidirectional UDP forwarder between client and auth server
	...
	go func() {
		...
		for {
			...
			if addr.String() == authAddr {
				// Response from auth server -> forward to client
				if clientAddr != nil {
					resolver.WriteTo(buf[:n], clientAddr)
				}
			} else {
				// Query from client -> forward to auth server
				clientAddr = addr
	...
	// XDNS server wrapping the auth server socket
	...
	// XDNS client configured to use the mock resolver
	config := &Config{
	...
	// Send test payload through client
	testPayload := []byte("hello xdns resolver")
	_, err = client.WriteTo(testPayload, rawConn.LocalAddr())
	if err != nil {
		t.Fatal(err)
	}

	// Read from server with timeout (server.ReadFrom blocks on channel,
	// so SetReadDeadline won't work; use a goroutine instead)
	...
	go func() {
		defer close(done)
		readN, _, readErr = server.ReadFrom(serverBuf[:])
	}()
	...
	if readErr != nil {
		t.Fatalf("server ReadFrom: %v", readErr)
	}
	if !bytes.Equal(serverBuf[:readN], testPayload) {
