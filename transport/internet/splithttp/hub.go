...
func ListenXH(ctx context.Context, address net.Address, port net.Port, streamSettings *internet.MemoryStreamConfig, addConn internet.ConnHandler) (internet.Listener, error) {
	...
	if port == net.Port(0) { // unix
		l.listener, err = streamSettings.FinalMask.Listen(ctx, &net.UnixAddr{
			Name: address.Domain(),
			Net:  "unix",
		})
		if err != nil {
		...
		Conn, err := streamSettings.FinalMask.ListenPacket(context.Background(), &net.UDPAddr{
			IP:   address.IP(),
			Port: int(port),
		})
		if err != nil {
		...
		l.listener, err = streamSettings.FinalMask.Listen(ctx, &net.TCPAddr{
			IP:   address.IP(),
			Port: int(port),
		})
		if err != nil {
