...
type Config struct {
	...
	// Server-side keepalive interval (seconds) for the stream-down response.
	// 0 disables it (default). Completes scStreamUpServerSecs, which only
	// keeps the stream-up (upload) response alive.
	...
	// Meta key the client uses to signal that it understands stream-down
	// framing. Empty means the default "x_df".
	DownFrameKey string `protobuf:"bytes,31,opt,name=downFrameKey,proto3" json:"downFrameKey,omitempty"`
	// Client-side switch: request stream-down framing/keepalive. Off by default;
	// framing only activates when both peers opt in (see the negotiation).
	DownFrame     bool `protobuf:"varint,32,opt,name=downFrame,proto3" json:"downFrame,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Config) GetDownFrameKey() string {
	if x != nil {
		return x.DownFrameKey
	}
	return ""
}

func (x *Config) GetDownFrame() bool {
	if x != nil {
		return x.DownFrame
	}
	return false
...
const file_transport_internet_splithttp_config_proto_rawDesc = "" +
	...
	"\x10hKeepAlivePeriod\x18\x06 \x01(\x03R\x10hKeepAlivePeriod\"\xf6\r\n" +
	"\x06Config\x12\x12\n" +
	...
	"\x16scStreamDownServerSecs\x18\x1e \x01(\v2..xray.transport.internet.splithttp.RangeConfigR\x16scStreamDownServerSecs\x12\"\n" +
	"\fdownFrameKey\x18\x1f \x01(\tR\fdownFrameKey\x12\x1c\n" +
	"\tdownFrame\x18  \x01(\bR\tdownFrame\x1a:\n" +
