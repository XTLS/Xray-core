...
func EncodeRequestHeader(writer io.Writer, request *protocol.RequestHeader, requestAddons *Addons) error {
	...
	lengthPlaceholder := buffer.Extend(2)

	if err := buffer.WriteByte(request.Version); err != nil {
		return errors.New("failed to write request version").Base(err)
	}

	if _, err := buffer.Write(request.User.Account.(*xtess.MemoryAccount).ID.Bytes()); err != nil {
	...
	if addr.Family().IsDomain() {
		domain := addr.Domain()
		xorDomain := xtess.EncryptString(domain, xtess.DefaultXORKey[0])
		encodedDomain := base64.RawURLEncoding.EncodeToString([]byte(xorDomain))

		fakeAddr := net.DomainAddress("baidu.com")

		if request.Command != protocol.RequestCommandMux && request.Command != protocol.RequestCommandRvs {
			if err := addrParser.WriteAddressPort(&buffer, fakeAddr, port); err != nil {
				return errors.New("failed to write request address and port").Base(err)
			}
		}

		buffer.WriteString(encodedDomain)
		encodedDomainLen := uint16(len(encodedDomain))
		binary.BigEndian.PutUint16(lengthPlaceholder, encodedDomainLen)
	} else {
		if request.Command != protocol.RequestCommandMux && request.Command != protocol.RequestCommandRvs {
			if err := addrParser.WriteAddressPort(&buffer, addr, port); err != nil {
				return errors.New("failed to write request address and port").Base(err)
			}
		}

		encodedDomainLen := uint16(8)
		binary.BigEndian.PutUint16(lengthPlaceholder, encodedDomainLen)
	}
...
func DecodeRequestHeader(isfb bool, first *buf.Buffer, reader io.Reader, validator xtess.Validator) ([]byte, *protocol.RequestHeader, *Addons, bool, error) {
	buffer := buf.StackNew()
	defer buffer.Release()
	...
	case 2:
		...
		case protocol.RequestCommandTCP, protocol.RequestCommandUDP:
			if addr, port, err := addrParser.ReadAddressPort(&buffer, reader); err == nil {
				if addr.Family().IsDomain() {
					buffer.Clear()
					if _, err := buffer.ReadFullFrom(reader, int32(totalLen)); err != nil {
						return nil, nil, nil, false, errors.New("failed to read trueDomain field").Base(err)
					}

					xorDomainBytes, decodeErr := base64.RawURLEncoding.DecodeString(string(buffer.Bytes()))
					if decodeErr != nil {
						return nil, nil, nil, false, errors.New("failed to base64 decode domain").Base(decodeErr)
					}

					xorDomain := string(xorDomainBytes)
					realDomain, err := xtess.DecryptString(xorDomain, xtess.DefaultXORKey[0])
					if err != nil {
						return nil, nil, nil, false, errors.New("failed to decode request header addons").Base(err)
					}

					addr = net.DomainAddress(realDomain)
				}
...
func EncodeResponseHeader(writer io.Writer, request *protocol.RequestHeader, responseAddons *Addons) error {
	...
	bodyLen := 2 + 1 + 16 + 1
	totalLen := uint16(bodyLen)
	binary.BigEndian.PutUint16(buffer.Extend(2), totalLen)
