...
func init() {
	common.Must(common.RegisterConfig((*Config)(nil), func(ctx context.Context, config interface{}) (interface{}, error) {
		...
		validator := new(xtess.MemoryValidator)
		for _, user := range c.Clients {
			u, err := user.ToMemoryUser()
			if err != nil {
				return nil, errors.New("failed to get XTESS user").Base(err).AtError()
			}
...
func New(ctx context.Context, config *Config, dc dns.Client, validator xtess.Validator) (*Handler, error) {
...
func (h *Handler) Process(ctx context.Context, network net.Network, connection stat.Connection, dispatcher routing.Dispatcher) error {
		...
		if isfb {
			...
			postRequest := func() error {
				...
				if fb.Xver != 0 {
					...
					case 2:
						pro.Write([]byte("\x0D\x0A\x0D\x0A\x00\x0D\x0A\x51\x55\x49\x54\x0A"))
						if ipType == 0 {
							pro.Write([]byte("\x20\x00\x00\x00"))
							break
						}
						if ipType == 4 {
							pro.Write([]byte("\x21\x11\x00\x0C"))
							pro.Write(net.ParseIP(remoteAddr).To4())
							pro.Write(net.ParseIP(localAddr).To4())
						} else {
							pro.Write([]byte("\x21\x21\x00\x24"))
	...
	if request.Command == protocol.RequestCommandUDP && (request.Port == 53 || request.Port == 853 || request.Port == 443) && xtess.DefaultXORKey != nil && len(xtess.DefaultXORKey) > 0 {
		if request.Address.Family().IsIP() {
			...
			errors.LogInfo(ctx, "XOR Decrypted UDP destination to ", request.Destination())
		}
	}

	if request.Command == protocol.RequestCommandTCP && (request.Port == 5222 || request.Port == 853 || request.Port == 443) && xtess.DefaultXORKey != nil && len(xtess.DefaultXORKey) > 0 {
		if request.Address.Family().IsIP() {
			ip := request.Address.IP()
			newIp := make([]byte, len(ip))
			copy(newIp, ip)
	...
	case "":
		inbound.CanSpliceCopy = 3
		if account.Flow == xtess.XRV && (request.Command == protocol.RequestCommandTCP || isMuxAndNotXUDP(request, first)) {
	...
	} else if account.Flow == xtess.XRV {
	...
	if request.Command == protocol.RequestCommandRvs {
		...
		return r.NewMux(ctx, &transport.Link{Reader: clientReader, Writer: clientWriter})
	}
...
func (r *Reverse) Start() error { return nil }

func (r *Reverse) Close() error { return nil }

func (r *Reverse) SenderSettings() *serial.TypedMessage { return nil }

func (r *Reverse) ProxySettings() *serial.TypedMessage { return nil }
