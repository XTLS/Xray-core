...
- On Windows and Linux, when using the TUN inbound, `auto_outbounds_interface` (`AutoOutboundsInterface`) must be set to a real outbound interface or `auto`, so outbound sockets, including ICMP Echo forwarding, do not route back into the TUN device
