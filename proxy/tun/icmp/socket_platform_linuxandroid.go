...
func openEchoSocket(netProto tcpip.NetworkProtocolNumber, dstIP tcpip.Address) (*Socket, error) {
	...
	if runtime.GOOS == "linux" && allPermissionDenied {
		errs = append(errs, "Linux ICMP Echo requires either net.ipv4.ping_group_range to allow the Xray process group for udp4/udp6 ping sockets, or CAP_NET_RAW for raw ICMP sockets.")
	}
...
func socketCandidates(netProto tcpip.NetworkProtocolNumber, dstIP tcpip.Address) []socketConfig {
	...
	case header.IPv4ProtocolNumber:
		...
		return []socketConfig{
			{network: "udp4", controllerNetwork: "udp4", listenAddr: "0.0.0.0", remoteAddr: &stdnet.UDPAddr{IP: ip}, acceptLocalIdentifier: true},
			{network: "ip4:icmp", controllerNetwork: "ip4", listenAddr: "0.0.0.0", remoteAddr: &stdnet.IPAddr{IP: ip}},
		}
	case header.IPv6ProtocolNumber:
		ip := stdnet.IP(dstIP.AsSlice())
		return []socketConfig{
			{network: "udp6", controllerNetwork: "udp6", listenAddr: "::", remoteAddr: &stdnet.UDPAddr{IP: ip}, acceptLocalIdentifier: true},
