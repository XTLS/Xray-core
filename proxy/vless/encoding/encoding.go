...
func DecodeRequestHeader(isfb bool, first *buf.Buffer, reader io.Reader, validator *vless.Validator, address net.Addr) (*protocol.RequestHeader, *Addons, bool, []byte, error) {
	...
	if isfb {
		...
		if _, err := buffer.ReadFullFrom(reader, 1); err != nil {
			return nil, nil, false, nil, newError("failed to read request version").Base(err)
		}
	...
	case 0:
		...
		if isfb {
			...
			if _, err := buffer.ReadFullFrom(reader, 16); err != nil {
				return nil, nil, false, nil, newError("failed to read request user id").Base(err)
			}
		...
		if err != nil {
			return nil, nil, false, nil, newError("failed to decode request header addons").Base(err)
		}

		buffer.Clear()
		if _, err := buffer.ReadFullFrom(reader, 1); err != nil {
			return nil, nil, false, nil, newError("failed to read request command").Base(err)
		}
		...
		if request.Address == nil {
			return nil, nil, false, nil, newError("invalid request address")
		}

		return request, requestAddons, false, AccountUUID.Bytes(), nil
	default:
		return nil, nil, isfb, nil, newError("invalid request version")
	}
}
