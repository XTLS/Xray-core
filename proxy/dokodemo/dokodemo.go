...
func init() {
	common.Must(common.RegisterConfig((*Config)(nil), func(ctx context.Context, config interface{}) (interface{}, error) {
		...
		err := core.RequireFeatures(ctx, func(pm policy.Manager, trackerSvc connectiontracker.Feature) error {
			return d.Init(config.(*Config), pm, session.SockoptFromContext(ctx), trackerSvc.Manager())
		})
		return d, err
	}))
}
...
func (d *DokodemoDoor) Init(config *Config, pm policy.Manager, sockopt *session.Sockopt, accessManager *connectiontracker.Manager) error {
